...
@brief
    This module implements the PCISpy main program
@todo
    - add /p page-by-page
    - add capability view
...
@mainpage
    PCISpy for all platforms, chipset independant
...
char gfPCI = 1; /*!< PCI 256 header only*/
...
/*!
    ...
    @return byte read
*/
unsigned long GetPci8(void* pAddr)
{
    unsigned long nRet;
...
/*!
    ...
    @return word read
*/
unsigned long GetPci16(void* pAddr)
{
    unsigned long nRet;
    unsigned reg = (unsigned long long)pAddr & 0xFF;        // register
    unsigned bdf = (unsigned)((unsigned long long)pAddr >> 12);         // bus dev fun at CF8
...
/*!
    ...
    @return dword read
*/
unsigned long GetPci32(void* pAddr)
{
    unsigned long nRet;
    unsigned reg = (unsigned long long)pAddr & 0xFF;        // register
    unsigned bdf = (unsigned)((unsigned long long)pAddr >> 12);         // bus dev fun at CF8
...
/*!
    ...
    @return dword read
*/
unsigned long SetPci32(void* pAddr,unsigned value)
{
    unsigned long nRet;
    unsigned reg = (unsigned long long)pAddr & 0xFF;        // register
    unsigned bdf = (unsigned)((unsigned long long)pAddr >> 12);         // bus dev fun at CF8
    ...
    for (bus = 0; bus < 256; bus++) {
        for (dev = 0; dev < 32; dev++) {
            for (fun = 0; fun < 8; fun++) {
                ...
                if ((unsigned)-1 == GetPci32((void*)((unsigned)pciebase + (bus << 20) + (dev << 15) + (fun << 12) + 0)))    // continue when no device found (0xFFFFFFFF)
                    continue;

                (*pfnHandler)(bus, dev, fun, pHandlerParm);  // invoke external handler 

                HeaderType = GetPci32((void*)((bus << 20) + (dev << 15) + (fun << 12) + 0xC));			        // check header type ...
        ...
        if (1) {
            ...
            for (i = 0; i < argc && 0 == fDone; i++) {
                static struct {
                    ...
                    { /*CommandLineString*/"/dump8" , /*size minus one */1 - 1, /*function to get element */&GetPci8 },
                    { /*CommandLineString*/"/dump16", /*size minus one */2 - 1, /*function to get element */&GetPci16 },
                    { /*CommandLineString*/"/dump32", /*size minus one */4 - 1, /*function to get element */&GetPci32 },
                    { /*CommandLineString*/"/dump"  , /*size minus one */4 - 1, /*function to get element */&GetPci32 }
                };
                int iDump;
