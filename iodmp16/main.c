...
@brief 
    This module implements the I/O DUMP 16/word main program
@todo
...
#define FORMATW_ADDR "%016X: %04X%s"
#define FORMATWOADDR "%s%04X%s"
...
int main(int argc, char** argv) {
    ...
    for (i = 0; i < (end - start) / TYPESIZE; i++)
        printf((i % (16 / TYPESIZE) == 0 ? FORMATW_ADDR : FORMATWOADDR), (i % (16 / TYPESIZE) == 0 ? (void*)(start + i * TYPESIZE) : ""), TYPEMASK & inpw(start + i * TYPESIZE), ((i + 1) % (16 / TYPESIZE)) ? (((i + 1) % (8 / TYPESIZE)) ? " " : " - ") : "\n");
